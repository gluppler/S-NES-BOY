...
- ✅ **NES**: Primary focus
- ✅ **Famicom**: Fully compatible (same PPU and interrupt system)
- ⚠️ **SNES**: Conceptually similar but different implementation (SNES has VBlank IRQ, different timing)

**Note**: SNES uses VBlank IRQ (interrupt request) instead of NMI, and has different scanline counts and timing. This document covers NES/Famicom NMI/VBlank only.
