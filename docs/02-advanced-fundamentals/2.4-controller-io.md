...
- ✅ **NES**: Primary focus
- ✅ **Famicom**: Fully compatible (same controller interface, though Famicom has hardwired controllers)
- ⚠️ **SNES**: Conceptually similar but different implementation (SNES uses different controller protocol and additional buttons)
...
**Note**: SNES controllers use a different communication protocol and have more buttons (X, Y, L, R, shoulder buttons). This document covers NES/Famicom controllers only.

## Hardware Behavior

NES and Famicom controllers use a **serial shift register** interface. Each controller has 8 buttons, represented as 8 bits that are shifted out one at a time.
