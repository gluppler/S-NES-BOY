...
- ✅ **NES**: Primary focus
- ✅ **Famicom**: Fully compatible (same hardware constraints)
- ⚠️ **SNES**: Partially applicable (concepts apply but SNES has different constraints - more RAM, faster CPU, different architecture)

**Note**: Optimization principles (zero page, lookup tables, register reuse) apply to SNES, but SNES has different hardware characteristics (24-bit addressing, more RAM, faster CPU) that change optimization priorities.
