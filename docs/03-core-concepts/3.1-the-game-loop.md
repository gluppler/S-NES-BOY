...
- ✅ **NES**: Primary focus
- ✅ **Famicom**: Fully compatible (same game loop architecture)
- ⚠️ **SNES**: Conceptually similar but different implementation (SNES uses VBlank IRQ instead of NMI, different timing)

**Note**: SNES game loop follows similar patterns (main loop + VBlank handler) but uses VBlank IRQ instead of NMI and has different timing characteristics. This document covers NES/Famicom game loop patterns.
