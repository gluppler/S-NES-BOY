...
- ✅ **NES**: Primary focus
- ✅ **Famicom**: Fully compatible (same PPU and rendering architecture)
- ❌ **SNES**: Does not apply (SNES has different PPU architecture with multiple layers and different rendering pipeline)

**Note**: SNES rendering uses different architecture (multiple background layers, Mode 7, different sprite system). This document covers NES/Famicom rendering only.
