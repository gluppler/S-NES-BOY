...
- ✅ **NES**: Primary focus
- ✅ **Famicom**: Fully compatible (identical memory map)
- ❌ **SNES**: Does not apply (SNES has different memory map with 24-bit addressing)

**Note**: SNES memory map is completely different (128 KB RAM, different address ranges, 24-bit addressing). This cheatsheet covers NES/Famicom memory only.
