...
Advanced optimization on the NES requires understanding CPU cycle counts, memory access patterns, and hardware constraints. Optimization is not premature—on a 1.79 MHz CPU with 2 KB RAM, every cycle and byte matters. This document covers proven techniques for maximizing performance while maintaining code clarity and hardware accuracy.
...
### 5. Hex String Macros for Multibyte Constants

**Rule: Use hex string macros to define multibyte constants in a readable format.

**Problem: Defining large multibyte constants (like lookup tables, tile data, or magic numbers) using individual `.byte` directives is verbose and error-prone.

**Solution: Use a hex string macro that converts readable hex strings into byte values.

**Example:
```asm
; ❌ BAD: Verbose byte-by-byte definition
magic_number:
    .byte $11, $22, $33, $44, $4F, $FF

; ✅ GOOD: Hex string macro (readable and maintainable)
.include "macros/hex_string.inc"

magic_number:
    hex "112233444FFF"        ; Same result, more readable

; Even better with separators for readability
lookup_table:
    hex "09F9_1102_9D74"      ; Underscores ignored, improves readability
    hex "E35B_D841_56C5"      ; Easy to see byte boundaries
    hex "6356_88C0"           ; Clear structure
```

**Benefits:**
- **Readability**: Hex strings are easier to read than long `.byte` lists
- **Maintainability**: Easy to modify hex values without counting commas
- **Flexibility**: Supports underscores and single quotes for formatting
- **Error Prevention**: Macro validates hex digits and ensures even number of digits

**Implementation:** The macro is available in `templates/nes/macros/hex_string.inc` and can be included in any NES project.

**Use Cases:**
- Defining lookup tables with hex values
- Specifying magic numbers or constants
- Creating tile data patterns
- Defining multibyte identifiers or signatures

### 6. Lookup Tables vs Computation
...
- [ ] Use hex string macros for readable multibyte constants
...
- [3.2 Data-Oriented Design](../concepts/3.2-data-oriented-design.md) - Memory layout optimization
- [3.3 Rendering Architecture](../concepts/3.3-rendering-architecture.md) - PPU update optimization
- [4.4 Timing Cheatsheets](../cheatsheets/04-cheatsheets/4.4-timing-cheatsheets.md) - Quick timing reference
