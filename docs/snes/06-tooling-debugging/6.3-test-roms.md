...
Test ROMs are available in the S-NES-BOY framework under `tests/snes/` (when available):
- **Location**: `tests/snes/` - SNES test ROM collection (directory structure ready)
- **Purpose**: Hardware-accurate test suites for verification and validation
- **Usage**: Reference implementations for SNES behavior

External test ROM sources:
- CPU tests
- APU tests
- PPU tests
