...
; Based on snes-hello canonical implementation
...
.byte "HELLO WORLD SNES" ; ROM name
...
@loop:
   ...
   ; SNES CGRAM format: 0bbbbbgg gggrrrrr (15-bit RGB, dual write)
   ; For white (R=31, G=31, B=31): low=$7F, high=$7F
...
@charset_loop:
   ...
   sta VMDATAH ; color index high bit set -> white (color 2)
   inx
   cpx #(128*8)
   bne @charset_loop
...
@numbers_line:
   ...
   ldx #(VRAM_BG1 + 32*(START_Y + 2) + START_X)
   stx VMADDL
   ldx #0
@numbers_loop:
   lda numbers_str,x
...
@punctuation_line:
   ...
   ldx #(VRAM_BG1 + 32*(START_Y + 3) + START_X)
   stx VMADDL
   ldx #0
@punctuation_loop:
