...
- ✅ **SNES: Primary focus (ca65/ld65 for 65816)

**Note: ca65 fully supports 65816 CPU (SNES) with `--cpu 65816` flag. SNES development uses the same toolchain as NES, with SNES-specific linker configurations and emulator (higan).
...
## SNES-Specific Toolchain Usage

### ca65 with 65816 Support

**SNES assembly file** (`game.asm`):
```asm
.feature force_range
.cpu "65816"
...
### SNES Linker Configuration

**SNES linker config** (`snes.cfg`):
```
MEMORY {
...
**higan** is the recommended SNES emulator for development:
