...
- ✅ **SNES: Primary focus (higan, bsnes)

**Note: SNES development uses higan (recommended) or bsnes for cycle-accurate emulation and debugging. This document covers both NES/ and SNES emulators.
...
**higan** is a highly accurate multi-system emulator, recommended for SNES development.
...
# See: https://higan.readthedocs.io/en/stable/install/linux/
```
...
**Command Line:**
```bash
# Launch higan with ROM file
higan game.sfc

# Fullscreen mode
higan --fullscreen game.sfc
```

**Per higan documentation:** higan can be launched with a ROM file as argument. The emulator will automatically detect the system type (SNES, NES, etc.) based on the ROM header.

### Debugging Features

**Memory Editor:**
- Tools → [Console] → Memory Editor
- View and edit WRAM, VRAM, CGRAM, OAM
- Per higan docs: Access via Tools menu

**PPU Viewer:**
- Tools → [Console] → PPU Viewer
- View background layers, sprites, palettes in real-time
- Useful for debugging graphics issues

**CPU Debugger:**
- Tools → [Console] → CPU Debugger
- View 65816 registers, disassembly, breakpoints
- Step through code execution

### Common Debugging Tasks (SNES)

**Find why sprite isn't appearing:**
1. Open PPU Viewer (Tools → Super Famicom → PPU Viewer)
2. Check OAM data (Tools → Super Famicom → Memory Editor → OAM)
3. Verify sprite Y position (should be 0-223 for NTSC)
4. Check TM register ($212C) — sprites enabled?

**Find why tile is wrong:**
1. Open Memory Editor → VRAM
2. Check tile data at VRAM address (per BG12NBA setting)
3. Verify tilemap entries point to correct tiles
4. Check palette in CGRAM

**Find timing issue:**
1. Use CPU Debugger to set breakpoints
2. Set breakpoint at NMI start
3. Count cycles until next breakpoint
4. Verify NMI completes quickly

### higan Resources

- **Official Documentation:** https://higan.readthedocs.io/en/stable/
- **Command Line Interface:** https://higan.readthedocs.io/en/stable/interface/higan-cli/
- **User Interface Guide:** https://higan.readthedocs.io/en/stable/interface/higan/
- **Installation Guide:** https://higan.readthedocs.io/en/stable/install/linux/
